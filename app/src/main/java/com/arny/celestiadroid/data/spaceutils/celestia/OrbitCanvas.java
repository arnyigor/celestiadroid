package com.arny.celestiadroid.data.spaceutils.celestia;

import android.app.Activity;
import android.graphics.*;
import android.media.Image;

import com.arny.celestiadroid.data.spaceutils.astro.AstroConst;
import com.arny.celestiadroid.data.spaceutils.astro.AstroUtils;
import com.arny.celestiadroid.data.utils.DateTimeUtils;
import com.arny.celestiadroid.data.utils.MathUtils;

import static com.arny.celestiadroid.data.spaceutils.astro.AstroUtils.distanceConvert;
...
//		repaint();
	...
	/**
	 * Make AstroPlanet Orbit
	 */
	private void updatePlanetOrbit(ATime atime) {
		for (int i = AstroPlanet.MERCURY; i <= AstroPlanet.PLUTO; i++) {
			this.planetOrbit[i - AstroPlanet.MERCURY]
	...
	public void setDate(ATime atime) {
		...
		for (int i = 0; i < 9; i++) {
			planetPos[i] = AstroPlanet.getPos(AstroPlanet.MERCURY + i, atime);
		}
	}

	/**
	 * Switch AstroPlanet Name ON/OFF
	 */
...
//		og.setFont(fontPlanetName);
		...
		if (OrbitDisplay[0] || OrbitDisplay[10]) {
			drawPlanetOrbit(og, planetOrbit[AstroPlanet.PLUTO - AstroPlanet.MERCURY],
		...
		if (OrbitDisplay[0] || OrbitDisplay[9]) {
			...
			drawPlanetOrbit(og, planetOrbit[AstroPlanet.NEPTUNE - AstroPlanet.MERCURY],
					colorPlanetOrbitUpper, colorPlanetOrbitLower);
		}
		drawPlanetBody(og, planetPos[7], "Neptune");

		if (OrbitDisplay[0] || OrbitDisplay[8]) {
			drawPlanetOrbit(og, planetOrbit[AstroPlanet.URANUS - AstroPlanet.MERCURY],
					colorPlanetOrbitUpper, colorPlanetOrbitLower);
		}
		drawPlanetBody(og, planetPos[6], "Uranus");

		if (OrbitDisplay[0] || OrbitDisplay[7]) {
			drawPlanetOrbit(og, planetOrbit[AstroPlanet.SATURN - AstroPlanet.MERCURY],
					colorPlanetOrbitUpper, colorPlanetOrbitLower);
		}
		drawPlanetBody(og, planetPos[5], "Saturn");

		if (OrbitDisplay[0] || OrbitDisplay[6]) {
			drawPlanetOrbit(og, planetOrbit[AstroPlanet.JUPITER - AstroPlanet.MERCURY],
					colorPlanetOrbitUpper, colorPlanetOrbitLower);
		}
		...
		if (fZoom * 1.524 >= 7.5) {
			if (OrbitDisplay[0] || OrbitDisplay[5]) {
				...
				drawPlanetOrbit(og, planetOrbit[AstroPlanet.MARS - AstroPlanet.MERCURY],
						colorPlanetOrbitUpper, colorPlanetOrbitLower);
			}
		...
		if (fZoom * 1.000 >= 7.5) {
			if (OrbitDisplay[0] || OrbitDisplay[4]) {
				...
				drawEarthOrbit(og, planetOrbit[AstroPlanet.EARTH - AstroPlanet.MERCURY],
		...
		if (fZoom * 0.723 >= 7.5) {
			if (OrbitDisplay[0] || OrbitDisplay[3]) {
				drawPlanetOrbit(og, planetOrbit[AstroPlanet.VENUS - AstroPlanet.MERCURY],
						colorPlanetOrbitUpper, colorPlanetOrbitLower);
			}
